...
func (preflight *GithubActionPreflight) Start(_ context.Context, state *GithubActionPreflightState, request preflight_kit_api.StartPreflightRequestBody) (*preflight_kit_api.StartResult, error) {
	...
	if request.ExperimentExecution.Name != nil && strings.Contains(strings.ToLower(*request.ExperimentExecution.Name), "startfailure") {
		return &preflight_kit_api.StartResult{Error: new(preflight_kit_api.PreflightKitError{
...
func (preflight *GithubActionPreflight) Status(_ context.Context, state *GithubActionPreflightState) (*preflight_kit_api.StatusResult, error) {
	...
	if state.ExecutionName != nil && strings.Contains(strings.ToLower(*state.ExecutionName), "technicalerror") {
		return nil, new(extension_kit.ToError("This is a test error", errors.New("with some details")))
		...
		return &preflight_kit_api.StatusResult{
			Completed: true,
			Error:     &preflight_kit_api.PreflightKitError{Title: "Preflight says: NO!", Detail: new("because no"), Status: extutil.Ptr(preflight_kit_api.Failed)},
		}, nil
		...
		return &preflight_kit_api.StatusResult{
			Completed: true,
			Error:     &preflight_kit_api.PreflightKitError{Title: "Preflight says: Oh NO. Error!", Detail: new("because no"), Status: extutil.Ptr(preflight_kit_api.Errored)},
		}, nil
		...
		if state.Started != nil && time.Since(*state.Started).Seconds() > 10 {
			return &preflight_kit_api.StatusResult{
				Completed: true,
				Error:     &preflight_kit_api.PreflightKitError{Title: "Inflight says: NO!", Detail: new("because no"), Status: extutil.Ptr(preflight_kit_api.Failed)},
			}, nil
		}
	}
	return &preflight_kit_api.StatusResult{
		...
		Summary: new(preflight_kit_api.Summary{
