...
func GetAdviceDescriptionKubernetesDeploymentDependencies() advice_kit_api.AdviceDefinition {
	return advice_kit_api.AdviceDefinition{
		...
		Label:                     "Test Advice - Kubernetes Deployment Dependencies",
		...
		AssessmentQueryApplicable: "target.type=\"com.steadybit.extension_kubernetes.kubernetes-deployment\" AND k8s.dependencies IS PRESENT AND steadybit.loadtest=\"true\"",
		Category:                  new("Scalability"),
		Status: advice_kit_api.AdviceDefinitionStatus{
			ActionNeeded: advice_kit_api.AdviceDefinitionStatusActionNeeded{
				AssessmentQuery: "COUNT(k8s.dependencies) >= 1",
				...
				Validation: &[]advice_kit_api.Validation{
					{
						...
						Description:        new("This validation creates multiple an *experiments* based on target dependencies. This one is for ${each.value}."),
						ShortDescription:   "With dependency ${each.value}",
						Type:               advice_kit_api.EXPERIMENT,
						ForEachAttribute:   new("k8s.dependencies"),
						ExperimentTemplate: new(advice_kit_api.ExperimentTemplate(LoadFile("advice_loadtest_validation_experiment_template_dependency.json"))),
					}, {
						...
						Type:             advice_kit_api.EXPERIMENT,
						ForEachAttribute: new("k8s.dependencies"),
						Experiment:       new(advice_kit_api.ExperimentTemplate(LoadFile("advice_loadtest_validation_experiment_dependency.json"))),
					}, {
